...
const purple = "#7c1aaa";
const dark = "#3a3a3a";
export const codecComponentsContextDefaultValue = {
  components: {},
  colors: {
    address: blue,
    boolean: blue,
  ...
  classNamePrefix: "codec-components"
} satisfies CodecComponentsContextValue;

type CustomComponent<D> = (props: { data: D }) => JSX.Element;
export interface CodecComponentsContextValue {
  // @/react/components
  components: Partial<{
    // /codec
    codec: Partial<{
  ...
  colors: Record<Exclude<CodeProps["type"], "bracket" | undefined>, string> & {
    bracket: string[];
  };
  classNamePrefix: string;
}
export const CodecComponentsContext =
  createContext<CodecComponentsContextValue>(
    codecComponentsContextDefaultValue
  );
export const useCodecComponents = () => useContext(CodecComponentsContext);

export type CodecComponentsProviderProps = {
  children: React.ReactNode;
  components?: CodecComponentsContextValue["components"];
  colors?: Partial<CodecComponentsContextValue["colors"]>;
  classNamePrefix?: CodecComponentsContextValue["classNamePrefix"];
};
export function CodecComponentsProvider({
  ...
  const value = useCodecComponents();
  const newValue = {
    components: {
      codec: { ...value.components.codec, ...components?.codec },
      common: { ...value.components.common, ...components?.common }
    },
    colors: { ...value.colors, ...colors },
    classNamePrefix: classNamePrefix || value.classNamePrefix
  } satisfies CodecComponentsContextValue;
  return (
    <CodecComponentsContext.Provider value={newValue}>
      {children}
    </CodecComponentsContext.Provider>
  );
}
